...
class RosVersions:
    ros_version = [
        ...
        RosVersion('foxy',  'ROS2', RosVersion.STATUS_LTS, ['python3-colcon-common-extensions',]),
        RosVersion('galactic',  'ROS2', RosVersion.STATUS_LTS, ['python3-colcon-common-extensions',]),
        RosVersion('rolling',  'ROS2', RosVersion.STATUS_LTS, ['python3-colcon-common-extensions',]),
        RosVersion('humble',  'ROS2', RosVersion.STATUS_LTS, ['python3-colcon-common-extensions',]),
        RosVersion('eloquent',  'ROS2', RosVersion.STATUS_EOL, ['python3-colcon-common-extensions',]),
...
class Tool(BaseTool):
    ...
    def choose_and_install_ros(self):
        ...
        if code==2:
            cmd_result = CmdTask("sudo apt install {} -y".format(dic_base[install_version]),300,os_command=True).run()
            cmd_result = CmdTask("sudo apt install {} -y".format(dic_base[install_version]),300,os_command=False).run()
        elif code==1:
            cmd_result = CmdTask("sudo apt install ros-{}-desktop -y".format(install_version),300,os_command=True).run()
            cmd_result = CmdTask("sudo apt install ros-{}-desktop -y".format(install_version),300,os_command=False).run()
